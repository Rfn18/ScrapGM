...
def playwright_scrape_single(kw, loc, ctr, limit, lat, lng,
                              extract_email=False, email_blacklist=None, chain_blacklist=None):
    """
    Scrape satu kota+keyword pakai Playwright.
    Aman dipanggil paralel dari ThreadPoolExecutor — TIDAK menyentuh
    st.session_state / st.* di dalam fungsi ini (itu penyebab
    'missing ScriptRunContext'). Koordinat (lat, lng) harus sudah
    di-resolve duluan di main thread sebelum fungsi ini dipanggil.

    Return:
        {"blocked": bool, "data": list[dict]}
    """
    results = []
            ...
            for gmaps_url in urls:
                ...
                try:
                    ...
                    if "/sorry/" in page.url:
                        return {"blocked": True, "data": results}
                    ...
                    try:
                        star_elements = page.locator(
                            '//span[contains(@aria-label, "stars") or contains(@aria-label, "bintang")]'
                        ).all()
                        for el in star_elements:
                            ...
                            if rating_text:
                                ...
                                if match:
                                    ...
                                    break
                        if rating == "N/A":
                            img_stars = page.locator(
                                'div[role="img"][aria-label*="stars"], div[role="img"][aria-label*="bintang"]'
                            )
                    ...
                    review_count = "N/A"
                    try:
                        rev_els = page.locator(
                            '//span[contains(@aria-label, "review") or contains(@aria-label, "ulasan") '
                            'or contains(text(), "review") or contains(text(), "ulasan")]'
                        ).all()
                        if not rev_els:
    ...
    return {"blocked": False, "data": results}